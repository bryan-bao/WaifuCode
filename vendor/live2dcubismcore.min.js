...
/* ------------------------------------------------------------------------
 * 下面这段不是 Live2D 官方的，是我们自己贴上去的兼容垫片。换 Core 记得带上。
 *
 * 为什么要它：Core 6.0.1 把每片网格的渲染次序从 `model.drawables.renderOrders`
 * 挪到了 `model.renderOrders`（因为 5.2 之后多了 offscreen，次序表要把
 * drawable 和 offscreen 排在一起）。而 pixi-live2d-display 打包的那份 Framework
 * 还是 Cubism 4 时代的，每帧都去读 drawables.renderOrders —— 读到 undefined，
 * 于是 doDrawModel 里 `e[i]` 当场抛 TypeError，**每一帧抛一次**。
 *
 * 表现很有迷惑性：人还在、还会动，但自检里跟渲染沾边的断言成片地挂
 *（视线、眼皮、藏部件全都失败），而错误只在 renderer 控制台里，日志上只有一行
 * "Cannot read properties of undefined (reading '0')"。
 *
 * 为什么非要 Core 6：Ren 是 Cubism 5.3 导出的，moc3 已经到第 6 版，
 * 5.1.0 的 Core 打不开（官方 SDK 5-r.4 是 5.1.0，r.5 直接跳到 6.0.1，
 * 中间没有既认 moc3 v6 又留着 renderOrders 的版本）。
 *
 * 垫片贴在 Core 上而不是贴在 5 个加载 cubism4.min.js 的地方，是因为这样
 * 桌宠本体和 tools/ 下那些自检脚本一次都不用改。
 * ---------------------------------------------------------------------- */
(function () {
  if (typeof Live2DCubismCore === 'undefined') return;
  var M = Live2DCubismCore.Model;
  if (!M || !M.fromMoc || M.__renderOrderShim) return;
  M.__renderOrderShim = true;

  var origFromMoc = M.fromMoc;
  M.fromMoc = function (moc) {
    var m = origFromMoc.call(this, moc);
    if (m && m.drawables && !('renderOrders' in m.drawables)) {
      Object.defineProperty(m.drawables, 'renderOrders', {
        get: function () {
          var all = m.renderOrders;
          var n = m.drawables.count;
          if (!all || all.length === n) return all;
          // 这个模型带 offscreen：次序表里混着 offscreen 的名次，直接给旧
          // Framework 会让它往数组外面写。只留网格那几项，名次重新压回 0..n-1。
          // （旧 Framework 本来就不会画 offscreen，这里只求别崩、别漏。）
          var idx = [];
          for (var i = 0; i < n; i++) idx.push(i);
          idx.sort(function (a, b) { return all[a] - all[b]; });
          var out = new Int32Array(n);
          for (var r = 0; r < n; r++) out[idx[r]] = r;
          return out;
        },
      });
    }
